...
export default function ChordVariationsPage() {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to load chord variations.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleEdit = () => {
    ...
    router.push(
      `/home/chords/edit/${currentVariation.id}?returnTo=${returnTo || ""}`,
    );
  };
  ...
  const handleDeleteSuccess = () => {
    if (variations) {
      const updatedItems = variations.items.filter(
      ...
      if (updatedItems.length > 0) {
        setVariations({
          ...variations,
          items: updatedItems,
          totalCount: variations.totalCount - 1,
        });
  ...
  const canEdit =
    currentVariation && user?.id === currentVariation.createdByUserId;
  const canDelete =
    currentVariation &&
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="font-medium">
                          {currentVariation.createdByNikName || "Unknown"}
                        </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground mt-2">
                          Reading from 6th string (thickest) to 1st string
                          (thinnest)
                        </div>
                      </div>
                      ...
                      <div className="mt-4 p-3 bg-muted/30 rounded text-sm">
                        ...
                        <div className="font-mono space-y-1">
                          <div>
                            6th string (E): {currentFingeringValues[0] || "0"} →{" "}
                            {currentFingeringValues[0] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[0]}`}
                          </div>
                          <div>
                            5th string (A): {currentFingeringValues[1] || "0"} →{" "}
                            {currentFingeringValues[1] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[1]}`}
                          </div>
                          <div>
                            4th string (D): {currentFingeringValues[2] || "0"} →{" "}
                            {currentFingeringValues[2] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[2]}`}
                          </div>
                          <div>
                            3rd string (G): {currentFingeringValues[3] || "0"} →{" "}
                            {currentFingeringValues[3] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[3]}`}
                          </div>
                          <div>
                            2nd string (B): {currentFingeringValues[4] || "0"} →{" "}
                            {currentFingeringValues[4] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[4]}`}
                          </div>
                          <div>
                            1st string (E): {currentFingeringValues[5] || "0"} →{" "}
                            {currentFingeringValues[5] === "0"
                              ? "Open"
                              : `Fret ${currentFingeringValues[5]}`}
                          </div>
                        </div>
                      </div>
                    </div>
