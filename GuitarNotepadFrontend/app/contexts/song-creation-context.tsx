...
import { SongCreationState, ToolMode, SongChordDto, SongPatternDto, UISegment, UIComment } from '@/types/songs';
...
type SongCreationAction =
  ...
  | { type: 'REPLACE_CHORD'; payload: { oldId: string; newId: string; chord: SongChordDto } }
  | { type: 'REPLACE_PATTERN'; payload: { oldId: string; newId: string; pattern: SongPatternDto } }
      ...
      return {
        ...
        segments: state.segments.map(segment => 
          segment.chordId === action.payload.oldId 
            ? { ...segment, chordId: action.payload.newId, color: action.payload.chord.color }
            : segment
        ),
        selectedChordId: state.selectedChordId === action.payload.oldId ? action.payload.newId : state.selectedChordId
      };
      ...
      return {
        ...
        segments: state.segments.map(segment => 
          segment.patternId === action.payload.oldId 
            ? { ...segment, patternId: action.payload.newId, backgroundColor: action.payload.pattern.color }
            : segment
        ),
        selectedPatternId: state.selectedPatternId === action.payload.oldId ? action.payload.newId : state.selectedPatternId
      };
...
export function useSongCreation() {
  ...
  if (context === undefined) {
    throw new Error('useSongCreation must be used within a SongCreationProvider');
  }
  return context;
}