...
public class ReviewsController : ControllerBase
{
    ...
    [HttpPost("likes")]
    public async Task<ActionResult<ReviewLikeDto>> CreateLike([FromBody] CreateReviewLikeDto dto)
    {
        try
        {
            var userId = GetCurrentUserId();

            var command = new CreateReviewLikeCommand(
                userId,
                dto.ReviewId,
                dto.IsLike);
            ...
            return CreatedAtAction(nameof(GetLike), new { id = result.Id }, result);
        }
        catch (ArgumentException ex)
        {
            return BadRequest(new { error = ex.Message });
    ...
    [HttpGet("likes/{id}")]
    public async Task<ActionResult<ReviewLikeDto>> GetLike(Guid id)
    {
        try
        {
            var query = new GetReviewLikeByIdQuery(id);
            var result = await _mediator.Send(query);
    ...
    public async Task<ActionResult<ReviewLikeDto>> ToggleLike([FromBody] CreateReviewLikeDto dto)
    {
        try
        {
            var userId = GetCurrentUserId();
    ...
    public async Task<ActionResult> DeleteLike(Guid id)
    {
        try
        {
            ...
            var command = new DeleteReviewLikeCommand(userId, id);
            await _mediator.Send(command);
